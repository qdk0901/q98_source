...
void codec_set_spk_rt3261(bool on)
{
...
#elif defined(CONFIG_SND_SOC_ES8323_PCM)
static int rt3261_modem_input_switch_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
...
static int rt3261_probe(struct snd_soc_codec *codec)
{
	...
	#if defined (CONFIG_SND_SOC_RT5623) || defined (CONFIG_SND_SOC_ES8323_PCM)
	#if 0 // es8323 use i2s0_clk as mclk
	struct clk *iis_clk;
	//for rt5623 MCLK use
