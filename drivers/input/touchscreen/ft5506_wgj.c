...
{
	struct FTS_TS_DATA_T *data = i2c_get_clientdata(this_client);
	struct ft5506_platform_data *pdata = this_client->dev.platform_data;
	u8 buf[64] = {0};
...
//					printk("[ERROR] Touch ID readed is illegal!! id=%d\n",id);
 ...
 //     y = (768-y);
				...
				else if((_st_finger_infos[id].u2_pressure == 0)||(touch_event == 1))//up
				{
					input_mt_slot(data->input_dev, id);
...
static unsigned char CTPM_FW[]=
{
	...
	#include "WGJ97090_2048X1536_V07_Q97S_app.h"
};
...
static void ft5x0x_ts_set_standby(struct i2c_client *client, int enable)
{
/* 
