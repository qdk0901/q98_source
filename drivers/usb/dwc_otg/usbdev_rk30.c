...
void usb20otg_hw_init(void)
{
        ...
        *otg_phy_con3 = (0x0300 << 16);
        ...
        gpio_request(get_otg_drv_pin(), NULL);
        gpio_direction_output(get_otg_drv_pin(), GPIO_LOW);
#else
...
{ 
    ...
    if(0 == enable)//disable
    {
        //gpio_set_value(RK3066B_OTG_DRV_VBUS, GPIO_LOW); 
				gpio_set_value(get_otg_drv_pin(), GPIO_LOW); 
    }
    if(1 == enable)//enable
    {
        //gpio_set_value(RK3066B_OTG_DRV_VBUS, GPIO_HIGH); 
        gpio_set_value(get_otg_drv_pin(), GPIO_HIGH); 
    }   
}
#endif
...
void usb20host_hw_init(void)
{
    ...
    gpio_request(get_host_drv_pin(), NULL);
    gpio_direction_output(get_host_drv_pin(), GPIO_HIGH);
#else
...
{ 
    ...
    if(1 == enable)//enable
    {
        ...
        gpio_set_value(get_host_drv_pin(), GPIO_HIGH);
