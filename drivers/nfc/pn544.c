		...
		size_t count, loff_t *offset)
{
  ...
  printk("[baker] irq_gpio low **********--irq=%d - \n",gpio_get_value(pn544_dev->irq_gpio));
	if (!gpio_get_value(pn544_dev->irq_gpio)) {
...
fail:
	mutex_unlock(&pn544_dev->read_mutex);
	printk("[baker] pn544_dev_read fail ----- \n");
	return ret;
}
		...
		const struct i2c_device_id *id)
{
	...
	ret = platform_data->request_resources();
	if (ret)
		return  -ENODEV;
	...
	pn544_dev->request_resources = platform_data->request_resources;
	pn544_dev->free_resources = platform_data->free_resources;
