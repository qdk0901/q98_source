...
static int batt_table[2*BATT_NUM+6] =
{
	...
	3303,	3531,	3688,	3796,	3874,	3911,	3938,	3967,	4009,	4091,	4200,
	3560,	3745,	3853,	3912,	3960,	4013,	4065,	4109,	4150,	4173,	4220,
};
...
7004,	7239,	7420,	7555,	7640,	7722,	7801,	7907,	8059,	8200,	8364,
#endif
};
#define adc_to_voltage(adc_val) ((adc_val * BAT_DEFINE_VALUE * (batt_table[4] +batt_table[5])) / (1024 *batt_table[5]))
#endif
...
static int rk30_adc_battery_suspend(struct platform_device *dev, pm_message_t state)
{
  ...
  if (data->pdata->set_charge_current != INVALID_GPIO) {
  	gpio_set_value(data->pdata->set_charge_current, GPIO_HIGH);	
  }
...
static int rk30_adc_battery_resume(struct platform_device *dev)
{
  ...
  if (data->pdata->set_charge_current != INVALID_GPIO) {
  	gpio_set_value(data->pdata->set_charge_current, GPIO_LOW);	
  }
	return 0;
}
#else
